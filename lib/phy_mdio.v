...
assign s_mdio_dr = s_mdio_dr_i & ~s_mdio_dv;
...
if ( PARALLEL ) begin
    ...
    assign mdc[0] = !s_mdio_dr_i ? mdio_clk : 1'b0;
    assign mdio[0] = !s_mdio_dr_i ? mdio_i : 1'bZ;
end
endgenerate
...
/*============================================================================*/
    ...
    if ( 2'b10 == mdio_clk_i ) begin
        if ( s_mdio_dv && s_mdio_dr_i ) begin
    ...
    if ( 2'b01 == mdio_clk_i ) begin
        if ( !s_mdio_dr_i ) begin // One idle bit
            mdio_busy <= 1;
        end
    end
        ...
        if ( mdio_busy ) begin
            ...
            if ( PREAMBLE && preamble ) begin
                ...
                if ( bit_count[0] ) begin
                    mdio_i <= s_mdio_a_i[15];
                    s_mdio_a_i <= {s_mdio_a_i[14:0], 1'b0};
                end
                if (( bit_count < 6'd36 ) && s_mdio_rd_i ) begin
                    mdio_i <= 1'bZ;
                    m_mdio_dv <= 0;
                    m_mdio_d_i <= 0;
                end
            end else if ( !s_mdio_rd_i ) begin
                if ( bit_count[0] ) begin
                    mdio_i <= s_mdio_d_i[15];
                    s_mdio_d_i <= {s_mdio_d_i[14:0], 1'b0};
                end
            end else begin
                mdio_i <= 1'bZ;
                if ( bit_count[0] ) begin
                    m_mdio_d_i <= {m_mdio_d_i[14:0], mdio[s_mdio_pa_i]};
                end
            end
        end
    end
